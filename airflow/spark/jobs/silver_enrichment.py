...
def run(spark: SparkSession, delta_dir: str, geolite2_db: str | None = None):
    ...
    geolite2_db : str or None
        ...
        will produce ``"Unknown"`` for all geo fields.
    """
    bronze_path = os.path.join(delta_dir, "bronze")
    ...
    # Initialise the GeoLite2 reader with the configured database path
    # (overrides the module-level default in utils/geoip.py).
    if geolite2_db:
...
def main():
    ...
    parser.add_argument(
        "--geolite2-db",
        default=None,
        help="Path to GeoLite2-City.mmdb file. If omitted, geo fields default to Unknown.",
    )
    ...
    try:
        run(spark, args.delta_dir, args.geolite2_db)
    finally:
        spark.stop()
