...
import base64, io, json, os, requests, shutil, subprocess, sys, tempfile
...
tmpdir = tempfile.mkdtemp(prefix="dbt_run_")

# Download dbt project ZIP from workspace (stored as workspace file)
import zipfile
...
dbt_dir = tmpdir
os.chdir(dbt_dir)
subprocess.run(["dbt", "deps", "--profiles-dir", tmpdir], capture_output=True)

print("Running dbt run...")
result = subprocess.run(["dbt", "run", "--profile", "w3c_azure",
