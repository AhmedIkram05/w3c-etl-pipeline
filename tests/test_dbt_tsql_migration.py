...
class TestCompiledAzureSqlOutput:
    ...
    These tests need compiled output generated by
    ``dbt compile --profile w3c_azure``.  If the compiled output was
    generated with the default ``postgres`` profile the tests skip
    automatically.
    """
    ...
    @pytest.fixture(autouse=True)
    def _require_tsql_compiled(self):
        """Skip all tests in this class when compiled output is from PostgreSQL."""
        p = self._find_compiled("dim_date.sql")
        if p is None:
            pytest.skip("No compiled output found at all. Run 'dbt compile --profile w3c_azure' first.")
        content = p.read_text(encoding="utf-8")
        if "::" in content:
            pytest.skip(
                "Compiled output contains PostgreSQL '::' casts — "
                "it was generated with the wrong profile. "
                "Run 'dbt compile --profile w3c_azure' to regenerate."
            )
